...
//Logic for when there is only one command. I am separating the
//logic for single commands and pipelining because I find it
//easier personally.
  if (cmd2 == NULL)
  {
  ...
  //If pipelining is used, we do not need to worry about adding
  //the commands to the job list or signal blocking since the 
  //commands will always be in the foreground.
  else
  {
...
int builtin_cmd(char **argv) 
{
  ...
  //Quit just needs to exit, jobs just needs to use listjobs.
  if (!strcmp(cmd, "quit") || !strcmp(cmd, "jobs"))
  {
    if(!strcmp(cmd, "quit"))
    {
      exit(0);
...
void do_bg(int jid) 
{
  ...
  //Change state, continue job, print the job.
  curJob->state = BG;
...
void do_fg(int jid) 
{
  ...
  //Change to FG, continue job, then wait for current
  //job to complete.
  curJob->state = FG;
  kill(-curJob->pid, SIGCONT);
  waitfg(curJob->pid);
...
void waitfg(pid_t pid)
{
  ...
  //If fg job changed state or has completed, we can exit the loop.
...
void sigchld_handler(int sig)
{
  ...
  //printf("%s", "reached sigchld\n");

  while((cur_pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0)
  {
    //If the child has exited, we can remove it from the jobs list
    //since we do not need to keep track of it anymore.
    if (WIFEXITED(status))
    {
      deletejob(jobs, cur_pid);
    }
    //If the child stopped, we need to update its status if it
    //hasn't been updated already. If it has, we do not need to
    //print the statement below.
    else if (WIFSTOPPED(status))
    {
      if (getjobpid(jobs, cur_pid) == NULL)
      {
        return;
      }

      if (getjobpid(jobs, cur_pid)->state == ST)
      {
        return;
      }

      sio_puts("Job [");
      sio_putl(pid2jid(cur_pid));
      sio_puts("] (");
      sio_putl(cur_pid);
    ...
    //If the child terminated from a signal, we also need to
    //remove it from the job list. If it has already been
    //removed, there is no need to print the statement.
    else if (WIFSIGNALED(status))
    {
      int jid = pid2jid(cur_pid);
      if (deletejob(jobs, cur_pid))
      {
        sio_puts("Job [");
        sio_putl(jid);
        sio_puts("] (");
...
void sigint_handler(int sig) 
{
  //Find the foreground job and forward the signal to its
  //process group. The job also needs to be removed from
  //the jobs list to indicate that it has exited.
...
{
  //Find the foreground job and forward the signal to its
  //proces group. The job's state needs to be changed to ST.
  for (int i = 0; i < MAXJOBS; i++)
  {
    if (jobs[i].state == FG)
    {
